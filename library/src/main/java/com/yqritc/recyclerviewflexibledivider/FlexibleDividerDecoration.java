...
public abstract class FlexibleDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    protected boolean mShowLastDivider; //是否是最后一条item
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        RecyclerView.Adapter adapter = parent.getAdapter(); //得到RecycleView的适配器
        if (adapter == null) {
            return;
        }

        int itemCount = adapter.getItemCount(); //得到item数量
        int lastDividerOffset = getLastDividerOffset(parent);
        int validChildCount = parent.getChildCount(); //一屏幕上的子条目数
    ...
    /**
     * 个人理解这个方法就是弥补item分隔线的宽度的方法,当分隔线画宽的时候,如果不调用这个方法分隔线就会占用item的宽度
     * @param rect
     * @param v
     * @param parent
     * @param state
